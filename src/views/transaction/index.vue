<template>
  <div class="pyzl-drawer">
    ...
    <template v-else>
      ...
      <div class="pyzl-drawer__footer">
        <div class="flex item-center justify-center">
          <template v-if="progressState === 1">
            <van-button class="footer--action" type="warning" style="width: 150px"
              @click="debounceActionHandler('appeal')">
              申诉
            </van-button>
            <van-button class="footer--action" type="primary" @click="debounceActionHandler('signRectificationPaper')"
              style="width: 150px">
              确认
            </van-button>
          </template>
          ...
          <template v-else-if="progressState === 2">
            <van-button plain type="primary" class="btn-action is--save"
          ...
          <template v-else-if="show.appealResult">
            ...
            <van-button @click="debounceActionHandler('rejectAppeal')" class="footer--action" type="primary"
              style="width: 180px">
              驳回申诉，签发整改
            </van-button>
          </template>
          ...
          <template v-else-if="show.rectificationLog">
            ...
            <van-button @click="debounceActionHandler('agreeAccept')" class="footer--action" type="primary"
              style="width: 180px">
              同意验收
            </van-button>
          </template>
          ...
          <template v-else>
            ...
            <van-button @click="debounceActionHandler('submit')" class="footer--action" type="primary"
              style="width: 150px">
              提交
            </van-button>
          </template>
      ...
      <!-- ## 申诉理由弹窗 -->
      <AppealReasonDialog title="申诉理由" v-model="show.appealReason" @appeal-succsss="actionHandler('appealSuccsss')">
      </AppealReasonDialog>

      <!-- ## 不予验收弹窗 -->
      <RejectAccpetDialog title="不予验收" v-model="show.rejectAccpet"></RejectAccpetDialog>
        ...
        <div class="p-15 rectificationPaper--section">
          <CardBox title="签收整改告知书" :show-toggler="false" type="simple">
            <div class="bg--white">
              ...
              <van-field v-model="form.rtfPeriod" center clearable label="整改要求:" placeholder="" :readonly="true">
              </van-field>

              <div class="van-cell van-cell--center van-field justify-between">
                <span>上传签收回执单:</span>
                <Uploader @upload-success="handleRectificationPaperUploadSuccess"></Uploader>
              </div>

              <div class="filelist rectificationReceiptList">
                <div class="van-cell" v-for="item in uploadList.rectificationReceiptList" :key="item.ossId">
                  {{ item.fileName }}
                </div>
              </div>
        ...
        <div class="p-15 carrierRectification--section">
          <CardBox title="承运商整改" :show-toggler="false" type="simple">
            <div class="bg--white p-15">
              <van-field v-model="form.carrierName" center clearable label="整改人:" placeholder="请输入整改人">
              </van-field>

              <van-field v-model="appealForm.rtfMark" center clearable label="整改描述:" placeholder="请输入整改描述">
              </van-field>

              <div class="van-cell van-cell--center van-field justify-between">
                <span>整改报告:</span>
                <Uploader @upload-success="handleAppealFormReportListUploadSuccess"></Uploader>
              </div>

              <div class="filelist appealFormReportList">
                <div class="van-cell" v-for="item in appealForm.reportList" :key="item.ossId">
                  {{ item.fileName }}
                </div>
              </div>
              ...
              <div class="van-cell van-cell--center van-field justify-between">
                <span>整改图片:</span>
                <Uploader @upload-success="handleAppealFormImageListUploadSuccess"></Uploader>
              </div>

              <div class="filelist appealFormImageList">
                <div class="van-cell" v-for="item in appealForm.imageList" :key="item.ossId">
                  {{ item.fileName }}
                </div>
              </div>
      ...
      <template v-if="show.appealResult">
        <div class="p-15 carrierRectification--section">
          <CardBox title="申诉结果" :show-toggler="false" type="simple">
            <div class="bg--white p-15">
              <van-field type="textarea" v-model="form.appealReason" center clearable label="申诉结果:" placeholder=""
                :disabled="true">
              </van-field>
              <p class="p-15 is--error">{{ appealForm.errorMsg }}</p>
            </div>
          ...
          <CardBox title="请选择整改类型" :show-toggler="false" type="simple">
            <div class="bg--white p-15">
              ...
              <van-radio-group v-model="radio">
                ...
                <van-field style="background-color: #f7f7f7;" v-show="isOtherRectificationRequirementType"
                  v-model="content" center clearable placeholder="请输入整改要求" />
              </van-radio-group>
            </div>
          </CardBox>
        </div>
      ...
      <template v-if="show.RectificationRequirementActionSheet">
        <!-- :state="this.preNode" -->
        <RectificationRequirementActionSheet :state="form.preNode" :close-on-click-overlay="false" :closeable="false"
          title="请选择整改类型" v-model="show.RectificationRequirementActionSheet">
...
export default {
  components: {
    ...
    ViolationInfoCard: () =>
      import(
        /* webpackChunkName: "violation-info-card" */ './ViolationInfoCard.vue'
      ),
    ...
    RejectAccpetDialog: () =>
      import(
        /* webpackChunkName: "reject-accpet-dialog" */ './RejectAcceptDialog.vue'
      ),

    // 整改签发弹窗
    RectificationRequirementActionSheet: () => import(
        /* webpackChunkName: "RectificationRequirement-ActionSheet" */ './RectificationRequirementActionSheet.vue'
    ),

    RectificationLogTable: () =>
      import(
        /* webpackChunkName: "acceptanceLog-table" */ './RectificationLogTable.vue'
      ),
  ...
  computed: {
    ...mapState({
      isLoading: state => state['@@loading'].effects['notification/updateNotificationDetailAsync'],
    }),
  ...
  methods: {
    ...mapActions({
      updateNotificationDetailAsync: 'notification/updateNotificationDetailAsync'
    }),
  ...
  created() {
    ...
    // 承运商申诉
    flowStateProcessingStrategies.setStrategy(
      4,
      function () {
